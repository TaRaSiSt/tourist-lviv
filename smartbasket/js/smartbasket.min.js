    ...
    t = jQuery, a = {
        ...
        showProducts: function (s) {
            let r = t("<form>").attr({
                ...
                }).html("Ціна за одну персону").append(p),
                u = t("<div>").attr({
            ...
            for (let n in s) {
                ...
                for (let a in s[n]) {
                    ...
                    if ("sbName" === a && (c.append(`<span>${s[n][a]}</span>`), p.val(s[n][a])), "sbSize" === a && void 0 !== s[n][a] && (c.children("span").append(`<span class="smart-basket__product-size"> ${s[n][a]}</span>`), d.val(s[n][a])), "sbPrice" === a && (_.html(s[n][a]), h.val(s[n][a])), "sbPriceCommon" === a) {
                        let t = +s[n][a];
                        y.val(+t.toFixed(2))
                    }
        ...
        stateBasket: function () {
            ...
            if (a.productType && a.productPrice) {
                let e = t("." + a.productType);
                t("." + a.productElement).find("." + a.productType + ":first-child").addClass(a.productType + "_active"), e.click(function (e) {
                    e.preventDefault(), t(this).parents("." + a.productElement).find("." + a.productType).removeClass(a.productType + "_active"), t(this).addClass(a.productType + "_active");
        ...
        commonResult: function (e, s, r) {
            let n = 0,
                ...
                }).text("Кошик пустий. Ви не додали жодної послуги").fadeOut(),
                l = t("<div>").attr({
                    class: "smart-basket__success-title"
                }).text("Бронювання успіше. Очікуйте дзвінка").css("display", "none"),
                c = t("<div>").attr({
            ...
            if (0 == +n ? o.fadeIn() : o.fadeOut(), r) t("." + l.attr("class")).css("display", "block"), setTimeout(() => {
                ...
                if (!s) return d.html(`<span>Кількість осіб: </span> ${n}`), p.html(`<span>Загальна ціна: </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`), c.append(o).append(l).append(d).append(p), c;
                0 == +n && t("." + o.attr("class")).css("display", "block"), t("." + d.attr("class")).html("<span>Кількість осіб: </span> "), t("." + d.attr("class")).html(`<span> Кількість осіб: </span> ${n}`), t("." + p.attr("class")).html("<span>Загальна ціна: </span> "), t("." + p.attr("class")).html(`<span>Загальна ціна: </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`)
            }
        },
        ...
        userForm: function () {
            ...
            if (c.append(p), a.agreement.isRequired) {
                ...
                r = a.agreement.isLink ? t("<a>").attr({
                    ...
                    class: "smart-basket__link-agreement"
                }), s.is(":checked") ? r.text("Оплатити онлайн") : r.text("Оплатити онлайн"), e.click(function () {
                    s.is(":checked") ? (r.text("Оплатити онлайн"), s.val("Оплатити онлайн"), m.attr({
                        disabled: "disabled"
                    }), d.addClass("smart-basket__input-wrapper_error")) : (r.text("Оплатити онлайн"), s.val("Оплатити онлайн"), m.attr({
                        disabled: !1
